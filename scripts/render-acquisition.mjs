...
export function renderAcquisitionSvg(rows, { capturedAt = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  const latestValue = latest ? latest.downloads.toLocaleString('en-US') : '—';
  const description = latest
    ? `Daily npm package downloads from ${first.day} through ${latest.day}. The latest completed day has ${latestValue} downloads, and the displayed range has ${total.toLocaleString('en-US')} downloads.`
  ...
  <title id="title">Daily npm package downloads for @ghostlygawd/codeweb</title>
  ...
  <text x="${width - 52}" y="92" text-anchor="end" fill="#C6F24E" font-family="ui-monospace,SFMono-Regular,Menlo,Consolas,monospace" font-size="58" font-weight="800">${xml(latestValue)}</text>
  <text x="${width - 52}" y="128" text-anchor="end" fill="#8A8794" font-family="ui-monospace,SFMono-Regular,Menlo,Consolas,monospace" font-size="17">latest completed day</text>
  ${grid}
  ...
  <text x="52" y="552" fill="#E8E7EE" font-family="ui-monospace,SFMono-Regular,Menlo,Consolas,monospace" font-size="22" font-weight="700">${total.toLocaleString('en-US')} downloads in the displayed range</text>
