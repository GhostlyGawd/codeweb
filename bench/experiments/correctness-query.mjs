...
//   H3     — file-level cycles      == independent Kosaraju SCC                 (bench/lib/oracles)
//   H4     — impact / blast radius  == independent reverse-reachability BFS      (bench/lib/oracles)
//   A-CALL — callers / callees      == independent raw call-edge neighbor sets   (oracles + _proptest)
//   A-TESTS— query --tests          == independent scan of test-edge in-neighbors(bench/lib/oracles)
//   A-CP   — context-pack window    == exactly the H4 impact set (no omissions)  (oracles)
//
// Rigor (see PRE-REGISTRATION.md (retired manuscript, git history @ v0.8.0) §0):
//   * Independent oracle: bench/lib/oracles.mjs is written from scratch and does NOT import
...
//
// Run: node bench/experiments/correctness-query.mjs   (writes bench/results/correctness-query.json)
...
async function main() {
  ...
  console.log(`results: bench/results/correctness-query.json`);
