...
- **`bench/experiments/efficiency-pilot.truth.json`** — hand-verified caller sets for all 4 targets,
...
+ per-agent transcripts (they survive in the session dir). New `bench/experiments/efficiency-pilot.usage.mjs`
...
- Re-run on any pilot run: `node bench/experiments/efficiency-pilot.usage.mjs --journal <path/to/wf_*.json>`.
...
## Key files
- Harness: `bench/experiments/efficiency-pilot.workflow.js` (Workflow script; 4 targets; control +
  treatment per target; oracle SKIPPED when `args.truth` supplied; `args.reps` → paired-delta mean±SD).
- **Frozen truth: `bench/experiments/efficiency-pilot.truth.json`** (lever #1; hand-verified, validated).
- **Regrade: `bench/experiments/efficiency-pilot.regrade.mjs`** → `efficiency-pilot.regrade.json`
  (deterministic; rescores committed runs vs frozen truth; symbol/file × full/indexed/external lenses).
- **Engine-frozen reps result: `bench/experiments/efficiency-pilot.reps8.json`** (8 reps, frozen truth,
...
- Rebuild pilot graphs (after any extractor change):
  `node scripts/run.mjs bench/corpus/axios --out-dir .codeweb/pilot/axios`
  `node scripts/run.mjs bench/corpus/flask --out-dir .codeweb/pilot/flask`
...
- Regrade committed runs vs frozen truth (no agents): `node bench/experiments/efficiency-pilot.regrade.mjs`.
