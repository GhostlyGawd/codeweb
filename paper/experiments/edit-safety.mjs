...
// Standalone: `node paper/experiments/edit-safety.mjs` runs every hypothesis end-to-end, prints one
// PASS/FAIL line per hypothesis, writes paper/results/edit-safety.json, and exits non-zero if ANY
// hypothesis misses its pre-registered criterion (so paper/run-all.mjs can gate on it).
//
