...
// ---- prompts --------------------------------------------------------------------------------------
const COMMON = `codeweb root: ${ROOT} (Node on PATH; use the Bash tool, forward-slash paths). The corpus repos are pinned read-only clones under bench/corpus/<repo>.`

const proposePrompt = `${COMMON}

You are the TASK PROPOSER for a fair A/B study. Read these corpus repos and propose ${cfg.tasksN * 2} candidate coding tasks spread across them: ${cfg.repos.map((r) => `bench/corpus/${r}`).join(', ')}.
...
Keep the best ${cfg.tasksN} (balanced across kinds + repos). WRITE the frozen kept set to ${ROOT}/bench/results/agent-ab-tasks.json as JSON {frozenAt:"pre-solve", tasks:[...], rejected:[{id,reason}]} (create bench/results/ if needed). Return kept, rejected (with reasons), frozenPath, and a one-line fairnessNote.`
...
STRICT ISOLATION: never edit bench/corpus/${t.repo} directly. First COPY it to a fresh throwaway dir:
  work=$(mktemp -d)/work ; cp -r "${ROOT}/bench/corpus/${t.repo}" "$work" ; rm -rf "$work/.git"
