#!/usr/bin/env node
// H18 — analyze the agent A/B raw cells (paper/results/agent-ab-raw.json) into agent-ab.json.
// Deterministic (the raw cells + seeded bootstrap are fixed); the A/B *solving* is not byte-repro
// (model nondeterminism) but this analysis of the committed raw data is. See agent-ab.DESIGN.md.
//   node paper/experiments/agent-ab-analyze.mjs
...
const raw = JSON.parse(readFileSync(join(ROOT, 'paper/results/agent-ab-raw.json'), 'utf8'));
...
writeFileSync(join(ROOT, 'paper/results/agent-ab.json'), JSON.stringify(out, null, 2));
...
console.log('[agent-ab] wrote paper/results/agent-ab.json');
