...
<p>The pre-registered agent A/B (H18): does a coding agent equipped with codeweb's pre-edit tools (<code>find-similar</code>, <code>placement</code>, <code>impact</code>, <code>simulate-edit</code>) introduce fewer structural regressions and less new duplication than the same agent without them? Nine tasks (add / refactor / fix across axios, flask, express) were proposed, adversarially screened for fairness, and <strong>frozen before any solver ran</strong>; each edit was graded by codeweb's own verified <code>diff.mjs</code> gate. ${abN('completion.completed', 34)} of ${abN('completion.cells', 36)} cells completed.</p>
<div class="callout"><p><strong>No measurable difference.</strong> Structural regressions averaged <strong>${abN('perCondition.treatment.structuralRegressions.mean', 0.11)} (treatment) vs ${abN('perCondition.control.structuralRegressions.mean', 0.13)} (control)</strong>; new duplication was <strong>0 in both arms</strong>; the paired bootstrap CI straddles 0 (Cliff's δ negligible). The null is <em>valid, not confounded</em> — <strong>all ${abN('toolEngagement.treatmentUsedTools', 18)} treatment cells used the tools</strong>, which answered correctly (<code>find-similar</code> reported no existing equivalent on the reuse tasks; <code>placement</code> confirmed the right directory). On clean, well-scoped tasks a capable base model already avoids regressions and duplication, so codeweb's pre-edit intelligence <em>corroborated rather than corrected</em> — no headroom (a floor effect). The effect on easy tasks is bounded near zero; it neither confirms nor refutes the hypothesis. Demonstrating corrective value would need higher-blast-radius tasks or a weaker base model — future work. Per the pre-registration the thesis rests on Themes 1–4; H18 is an honest pilot, not load-bearing.</p></div>
</section>
