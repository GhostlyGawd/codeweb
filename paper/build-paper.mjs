...
<p>Its structural analysis matched independent oracles in every one of ~490,000 trials, its detectors are accurate against labeled ground truth, and it is fast and dependency-free. Where it fell short, the evaluation said so — and in two cases that honesty produced a better tool, because the harnesses found defects the existing test suite did not. Outcomes over promises: the data is in <a href="${GH}/tree/main/paper/results">paper/results/</a>, and the deterministic results regenerate with one command.</p>
</section>
