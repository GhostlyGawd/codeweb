...
3. The surviving set (target **N ≈ 12–16**, balanced across kinds and repos) is **frozen** to
   `bench/results/agent-ab-tasks.json` **before any solver runs**. No task is added or dropped after
   solving begins.
...
`bench/experiments/agent-ab.mjs` (a workflow): stage 1 propose+freeze tasks → stage 2 solve each
(task,condition,rep) in a worktree → stage 3 grade each diff with `diff.mjs` → stage 4 paired stats.
Writes `bench/results/agent-ab.json` + `agent-ab-tasks.json` + per-run diffs under `bench/results/ab/`.
