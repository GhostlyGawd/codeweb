...
function extract() {
  const res = runNode(script('extract-symbols.mjs'), [SRC, '--target', 'defexp-x', '--no-ctags', '--out', OUT]);
  assert.equal(res.status, 0, `extractor exited non-zero:\n${res.stderr}`);
  return readJSON(OUT);
}

test('a default import of a single-symbol class lands the coarse edge on the CLASS', () => {
  const frag = extract();
...
test('the default-export import edge is NOT routed to <module> when a single symbol exists', () => {
  const frag = extract();
...
test('DEPENDENTS: the importer is a dependent of the class even without a detected call', () => {
  const frag = extract();
