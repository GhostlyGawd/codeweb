...
Mode: Review. This review does not change the source requirements and does not
...
| DR-007 | Review and approval | No digest-bound human review or baseline approval exists for the structured rewrite. | Obtain the required human reviews, correct the artifact, rerun the checker, and then obtain operator baseline approval against the final digest. | `REVIEW-001`, `APPROVAL-001` | Blocks clean output and baseline status. |

## Required decisions and reviewer actions

1. Decide whether AC-3 must keep an installation claim. If it must, approve an
   objective installation test and its success criteria.
2. Review the structured rewrite for source fidelity, coverage, conflicts,
   feasibility, and verification adequacy.
3. Correct all review findings and rerun the checker.
4. Record operator baseline approval only against the final content digest.
...
- [ ] Add the structured rewrite and traceability data.
- [ ] Run the deterministic checker and preserve its report.
- [ ] Reconcile specification, implementation, tests, and documentation.
- [ ] Obtain human requirements review.
- [ ] Obtain operator baseline approval.
...
| Product acceptance criteria | Required | Shipped behavior | Commands and pins named by `SPEC.md` | `SPEC.md` | Review findings DR-001 through DR-006. |
| Structured requirement records | Supporting trace artifact | No behavior change | Requirements checker | Planned companion YAML and generated report | In progress. |
| Installation and execution with zero required dependencies | Required source claim | Package and CLI behavior | Current AC-3 check inspects only `package.json` | `SPEC.md`, AC-3 | Required conflict between claim scope and evidence scope; open. |
...
The deterministic checker can validate structure and selected text patterns.
It cannot establish semantic correctness, feasibility, completeness, safety,
or source fidelity. A human review and operator approval remain mandatory.
